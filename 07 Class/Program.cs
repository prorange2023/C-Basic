namespace _07_Class
{
    internal class Program
    {
        ...
        // 클래스는 참조형식 구조체는 값 형식
        // 클래스가 그 악명높은 포인터의 C# 버전이다.
        class Student // 클래스  struct 로 바꿔도 똑같은데 차이가 뭔가
        {
            public string name;
            public int math;
            public int english;
        ...
        // <클래스 생성자>
        // 반환형이 없는 클래스이름의 함수를 생성자라 하며 이 생성자를 쓰는 이유는 클래스의 인스턴스를 만들 때 호출되는 역할로 사용
        // 인스턴스의 생성자는 new 키워드를 통해서 사용

        // <클래스 얕은복사>
        // 클래스에 대입연산자(=)를 통해 같은 인스턴스를 참조함
        // 구조체는 깊은복사 , 클래스는 얕은 복사
        class MyClass
        {
            public int value1;
            public int value2;
        ...
        void Main2()
        {
            ...
            Car truck = new Car("트럭", "파란색");  // 이게 새로 차를 만들고 그 위치를 가르키고 있는것
            Console.WriteLine($"{truck.name},{truck.color}"); // 가리키고 있는것의 이름과 컬러를 갖고 오는거다.
        }

        // 값이 필요할때가 아니라 Car 라는 녀석 자체가 중요할때 쓴다?
        ...
        {
            public RefType shallow; // 몬스터
            public RefType deep; // 아이템
